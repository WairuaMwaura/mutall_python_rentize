...
const my_cells = document.querySelectorAll("td");
// 
// Convert cells node list to an array.
const cells_arr = Array.from(my_cells);
// 
// Select all the table rows.
const my_rows = document.querySelectorAll("tr");
// 
// Convert cells node list to an array.
const rows_arr = Array.from(my_rows);
// 
// Select the last row of the table.
const last_row = document.querySelector('tr:last-child');
// 
// Loop through each cell and add an event listener to them.
cells_arr.forEach((clicked_cell) => {
    clicked_cell.onclick = function () {
        ...
        if (clicked_cell.closest('tr') != last_row) {
            ...
            if (clicked_cell.classList.contains('active-cell')) {
                ...
                const parentRow = clicked_cell.closest('tr');
                if (parentRow) {
                ...
                const parentRow_2 = clicked_cell.closest('tr');
                if (parentRow_2) {
