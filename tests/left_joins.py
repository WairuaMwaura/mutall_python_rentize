...
#
# Create a left join for all the active water connections and the current water
#   readings. Afterward, create another left join between the previous left-join
#   result and the previous water readings
# Get all active water connections in a DataFrame
def get_all_active_wconnectinons() -> DataFrame:
    #
    # Execute SQL statement on Database
    kasa.execute("select * from wconnection")
    #
    # Fetch the query results from the Database
    all_water_connections = kasa.fetchall()
    #
    # Create a DataFrame out of the fetched result
    water_connection_df = DataFrame(all_water_connections)
    #
    # Get active water connections based on end date
    filter_date = date(9999, 12, 31)
    active_water_connections_df = water_connection_df[
        water_connection_df['end_date'] == filter_date]
    return active_water_connections_df


# Store the active water connections, current readings and previous readings
#   DataFrames in separate variables
active_wconnections = get_all_active_wconnectinons()
curr_readings = get_current_readings()
prev_readings = get_previous_readings()

# left join
active_wconnections_curr = active_wconnections.merge(
    curr_readings,
    on='wconnection',
    how='left'
)
active_wconnections_curr_prev = active_wconnections_curr.merge(
    prev_readings,
    on='wconnection',
    how='left'
)

print(active_wconnections_curr_prev)
